...
auto Debugger::command_step(Args args) -> bool {
    switch (args.size()) {
        ...
        case 1:
            // TODO: clean this up
            // try/catch should be moved somewhere else (could use optional?)
            // Should be abled to avoid subtracting 1
            try {
                int nsteps = std::stoi(args[0]);
                if (nsteps == 0) {
                    log_error("Cannont step zero times");
                    return false;
                }
                if (nsteps < 0) {
...
void Debugger::command_memory(Args args) {
    ...
    u16 memory_location = static_cast<uint>(std::stoul(args[0], nullptr, 16));
...
void Debugger::command_help(const Args& args) {
    ...
    printf("[r]un                   Run $steps cycles\n");
    printf("breakaddr $addr         Run $steps cycles\n");
...
auto Debugger::parse(const std::string& input) -> Command {
    ...
    // If nothing entered repeat last command
